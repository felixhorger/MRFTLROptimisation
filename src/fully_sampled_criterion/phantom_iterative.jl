...
function simulate(name, dictionary_path, t0)
	...
	#
	num_runs = 10 # Averaging over instances of the sampling pattern
	kspaces_to_sample = num_σ:12
	errors = Array{Float64}(undef, num_σ, num_runs, length(kspaces_to_sample))
	conditionings = Matrix{Float64}(undef, num_runs, length(kspaces_to_sample))
	normalisation = sum(abs2, fullysampled; dims=1:2) # sum of |.|^2 over spatial dimensions
	Threads.@threads for n in 1:(num_runs*length(kspaces_to_sample))
		i = mod1(n, num_runs)
		j = (n-1) ÷ num_runs + 1
		num_kspaces = kspaces_to_sample[j]
		sampling = MRITrajectories.rand(Int, () -> (rand(), rand()), (num_kspaces * prod(shape)), shape, num_kspaces, num_kspaces)
		...
		@assert all(mask .== num_kspaces)
		lr_inversion_recon = reconstruct(sampling)
		errors[:, i, j] = dropdims(sqrt.(sum(abs2, fullysampled .- lr_inversion_recon; dims=(1,2))); dims=(1,2))
		conditionings[i, j] = MRF.calculate_conditioning(
			MRF.compute_eigenvalues(
				VH, shape,
				MRIRecon.split_sampling_spatially(sampling, shape, size(VH, 2))
			)
		)[1]
	end
	# TODO: change "recons" to errors
	jldsave("$(name)_$(num_time)_recons_iter.jld2"; errors, conditionings, kspaces_to_sample, normalisation)
	return
end
