...
random = 1e5 .* load("../../../20240219_Scanning/random/mrf.jld2", "recon")[:, :, 100, :]
optimised = 1e5 .* load("../../../20240219_Scanning/optimised/mrf.jld2", "recon")[:, :, 100, :]
...
caspr = 1e5 .* load("../../../20240219_Scanning/caspr/mrf.jld2", "recon")[:, :, 100, :]
caspr = circshift(caspr, (-8, 3, 0))
@views mask_caspr = circshift(MRIPhantoms.ellipsoidal_shutter(size(caspr)[1:2], (90, 90)), (2, 4)) #abs.(caspr[:, :, 1]) .> 0.45
@views mask_caspr_maps = circshift(MRIPhantoms.ellipsoidal_shutter(size(caspr)[1:2], (86, 86)), (5, 2)) #abs.(caspr[:, :, 1]) .> 0.45
#caspr_simulation = 4.0 * load("../../../20240219_Scanning/simulation/sim.jld2", "recon")
#fullysampled = 1.0 * load("../../../20230713_Scanning/simulation/recon.jld2", "fullysampled_recon")
mask_sim = circshift(MRIPhantoms.ellipsoidal_shutter(size(caspr)[1:2], (88, 88)), (0, 2))
mask_sim_maps = circshift(MRIPhantoms.ellipsoidal_shutter(size(caspr)[1:2], (86, 86)), (0, 2))
...
for s = 1:4
	...
	#axs[s, 4].imshow(scales[s] .* mask_sim .* abs.(caspr_simulation[:, :, s]); cmap="gray", vmin, vmax)
	for i = 1:3
...
TODO: single channel maps?
TODO: connect these below to the maps
...
for s = 1:2
	...
	#plot = axs[s, 3].imshow(mask_caspr .* caspr_maps[s][100, :, :]; cmap=cmaps[s], vmin=vmins[s], vmax=vmaxs[s])
	#cax, cbar = PyPlotTools.add_colourbar(fig, axs[s, 3], plot; size="8%", phantom=true)
	#add_stats(axs[s, 3], caspr_t12[s])
	plot = axs[s, 3].imshow(mask_caspr_maps .* caspr_corrected_maps[s][100, :, :]; cmap=cmaps[s], vmin=vmins[s], vmax=vmaxs[s])
	cax, cbar = PyPlotTools.add_colourbar(fig, axs[s, 3], plot; size="8%", phantom=true)
	add_stats(axs[s, 3], caspr_corrected_t12[s])
	#plot = axs[s, 4].imshow(mask_radial .* radial_maps[s][100, :, :]; cmap=cmaps[s], vmin=vmins[s], vmax=vmaxs[s])
